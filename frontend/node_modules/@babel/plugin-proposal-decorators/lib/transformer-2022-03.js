...
function transformClass(path, state, constantSuper, version) {
  ...
  originalClass.body.body.unshift(_core.types.staticBlock([_core.types.expressionStatement(createLocalsAssignment(elementLocals, classLocals, elementDecorations, classDecorations, state, version)), requiresStaticInit && _core.types.expressionStatement(_core.types.callExpression(_core.types.cloneNode(staticInitLocal), [_core.types.thisExpression()]))].filter(Boolean)));
  path.insertBefore(assignments.map(expr => _core.types.expressionStatement(expr)));
  path.scope.crawl();
  return path;
}
function createLocalsAssignment(elementLocals, classLocals, elementDecorations, classDecorations, state, version) {
  let lhs, rhs;
  if (version === "2021-12" || version === "2022-03" && !state.availableHelper("applyDecs2203R")) {
    ...
    rhs = _core.types.callExpression(state.addHelper(version === "2021-12" ? "applyDecs" : "applyDecs2203"), [_core.types.thisExpression(), elementDecorations, classDecorations]);
  } else {
    rhs = _core.types.callExpression(state.addHelper("applyDecs2203R"), [_core.types.thisExpression(), elementDecorations, classDecorations]);
...
//# sourceMappingURL=transformer-2022-03.js.map
